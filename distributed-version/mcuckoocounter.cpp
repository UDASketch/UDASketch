...
int Query(int hash1, int hash2, int fp, int r)
{
	...
	double min1=min(hg[r][hash[0]].count12&0xf, hg[r][hash[1]].count12&0xf);

	double min2=(min(hg[r][hash[0]].count12&0xf0, hg[r][hash[1]].count12&0xf0))>>4;
...
int MeanQuery(int hash1, int hash2, int fp)
{
	int query[range];
	for (int i = 0; i < range; i++)
		query[i] = Query(hash1, hash2, fp, i);
	int sumquery = 0;
	for (int i = 0; i < range; i++)
		sumquery += query[i];
	...
	for (int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt) {
		...
		for (int i = 0; i < test_cycles; i++) {
			...
			for (int i = 0; i < packet_cnt; i++) {
				...
				gb_cnt[hash] += 1;
				lc_cnt[hash] += 1;
				if (hit_cnt.count(hash) == 0) hit_cnt[hash] = MeanQuery(hash1, hash2, fp);
				...
				if(i && i % window == 0) {
					...
					if(1) {
						...
						for (auto it:gb_cnt) {
							...
							int hash1 = hash % BUCKET_NUM;
							int hash2 = (hash1 ^ BKDRHash((char *)&fp)) % BUCKET_NUM;
							int efreq = MeanQuery(hash1, hash2, fp);
							if (efreq >= th) f2 = 1;
							if (f1 && f2) tp++;
							else if (f1 && !f2) fn++;
							else if (!f1 && f2) fp++;
			...
			for (int i = 0; i < topk.size(); i++) {
				...
				int hash1 = hash % BUCKET_NUM;
				int hash2 = (hash1 ^ BKDRHash((char *)&fp)) % BUCKET_NUM;
				int efq = MeanQuery(hash1, hash2, fp);
