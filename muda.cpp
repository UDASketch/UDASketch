...
public:
			...
			case 2:
				{
					...
					if (num3 == usage3 && num2 >= 5) {
						...
						for (int i = 0; i < (num2+5)*2; i += 2) {
							if (heavy[i]) {
								if (heavy[i] == f)
									fp_fq.push_back(make_pair(make_pair(i, heavy[i]), fq));
								else 
									fp_fq.push_back(make_pair(make_pair(i, heavy[i]), heavy[i+1]));
							}
						}
						...
						for (int i = (num2+3)*2; i < (num2+3)*2+num3*5/2; i += 5) {
							if (heavy[i]) {
								if (heavy[i] == f)
									...
									fp_fq.push_back(make_pair(make_pair(i, heavy[i]), (heavy[i+1]<<4) + ((heavy[i+2]>>4)&0x15)));
							}
							if (heavy[i+4]) {
								if (heavy[i+4] == f)
									fp_fq.push_back(make_pair(make_pair(i+4, heavy[i+4]), fq));
								else
									fp_fq.push_back(make_pair(make_pair(i+4, heavy[i+4]), (heavy[i+3]<<4) + (heavy[i+2]&15)));
							}
						}
						sort(fp_fq.begin(), fp_fq.end(), cmp1);
						if (fp_fq.size() == 7) {
	...
	void output(int hash, uint8_t f)
	{
		...
		for (int j = start; j < end; j += 5) {
			uint8_t e = heavy[j];
			cout<<(int)e<<" "<<((int)(heavy[j+1]<<4)+((heavy[j+2]>>4)&15))<<endl;
			e = heavy[j+4];
			cout<<(int)e<<" "<<((int)(heavy[j+3]<<4)+(heavy[j+2]&15))<<endl;
		}
		...
		for (int j = start; j < end; j += 3) {
			uint8_t e = heavy[j];
			cout<<(int)e<<" "<<((int)(heavy[j+1]<<8) + (uint8_t)heavy[j+2])<<endl;
		}
	}
};
...
int MeanQuery(int hash, int shash, uint8_t f)
{
	int shashi[range];
	...
	int query[range];
	for (int i = 0; i < range; i++)
		query[i] = hg[i][hash].Query(shashi[i], f);
	//sort(query, query + range);
	//return max(query[range / 2], 0);
	int sumquery = 0;
	for (int i = 0; i < range; i++)
		sumquery += query[i];
	...
	for (int datafileCnt = START_FILE_NO; datafileCnt <= END_FILE_NO; ++datafileCnt) {
		...
		for (int t = 0; t < test_cycles; t++) {
			...
			for (int i = 0; i < packet_cnt; i++) {
				...
				//int hash = BKDRHash(keys[i], 8); //webpage
				uint8_t fp = finger_print(hash);
				int shash = SHash(hash);
				int hash1 = hash % BUCKET_NUM;
				gb_cnt[hash] += 1;
				lc_cnt[hash] += 1;
				if (hit_cnt.count(hash) == 0) hit_cnt[hash] = MeanQuery(hash1, shash, fp);
				for (int j = 0; j < range; j++)
					hg[j][hash1].Insert((shash>>j)&1, fp);
				...
				if(i && i % window == 0) {
					...
					if(1) {
						...
						for (auto it:gb_cnt) {
							...
							int shash = SHash(hash);
							int hash1 = hash % BUCKET_NUM;
							int efreq = MeanQuery(hash1, shash, fp);
							if (efreq >= th) f2 = 1;
							if (f1 && f2) tp++;
							else if (f1 && !f2) fn++;
							else if (!f1 && f2) fp++;
					...
					if(1) {
						...
						for (auto it:lc_cnt) {
							...
							int shash = SHash(hash);
							int hash1 = hash % BUCKET_NUM;
							int efreq = MeanQuery(hash1, shash, fp) - hit_cnt[hash];
							ph_cnt[hash] = efreq;
							if (efreq >= th) f2 = 1;
							if (f1 && f2) tp++;
							else if (f1 && !f2) fn++;
			...
			for (int i = 0; i < topk.size(); i++) {
				...
				int shash = SHash(hash);
				int hash1 = hash % BUCKET_NUM;
				int efq = MeanQuery(hash1, shash, fp);
